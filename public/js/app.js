...
$(function () {
  ...
  function getTweets(title) {
    ...
    var tweets = $.get('https://localhost:8000/api/tweets?q=' + title).done(function (data) {
      if (data.statuses.length === 0) {
        // Truncate the title
        title = title.split(",")[0];
        console.log("Truncated Title: ", title);
        tweets = $.get('https://localhost:8000/api/tweets?q=' + title).done(function (dataTweets) {
